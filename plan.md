...
### Architecture clarification add-on

The design will be organized into several explicit sub-modules: an AXI interface block, a controller, a 24-lane state register file, a round-constant add block, an S-box block, an MDS mixing block, modular arithmetic units, and squeeze/output logic. Separating these blocks makes the dataflow easier to verify and allows the controller to schedule each stage of the Poseidon round more clearly.

The PS/IP connection will use an AXI4-Lite interface for control and status and AXI4-Stream interfaces for input and output data movement. The AXI4-Lite interface will carry control information such as start, reset, operation mode, input length, and status signals such as busy and done. The AXI4-Stream input will carry 256-bit field elements into the accelerator during absorb, and the AXI4-Stream output will carry 256-bit hash words back to the PS during squeeze.

Inside the IP, the controller will send round number, lane select, operation select, and write-enable control signals to the datapath modules. The state register file will provide lane data to the round-constant, S-box, and MDS blocks, and the modular arithmetic units will return reduced results back into the state register file. This makes the module boundaries and interfaces more explicit while keeping the arithmetic resources shared across rounds.
